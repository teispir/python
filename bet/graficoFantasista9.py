...
# Aggiungiamo il valore dopo la cedola
values.append(1256.33)

# Creazione dell'array degli indici
times = np.arange(len(values))

# Trova minimo, massimo, pre-cedola e current
min_index = np.argmin(values)
max_index = np.argmax(values)
pre_cedola_index = len(values) - 2  # Penultimo valore: 1756.33
current_index = len(values) - 1     # Ultimo valore: 1256.33
...
# Evidenzia minimo, massimo, pre-cedola e current
highlight_indices = [min_index, max_index, pre_cedola_index, current_index]
highlight_values = [values[min_index], values[max_index], values[pre_cedola_index], values[current_index]]
colors = ['red', 'green', 'orange', 'purple']
labels = ['Min', 'Max', 'Pre-Cedola', 'Current']
...
for idx, val, color, label in zip(highlight_indices, highlight_values, colors, labels):
    ...
    plt.text(idx, val, f'{label}: {val:.2f}', verticalalignment='bottom' if label == 'Min' else 'top',
             horizontalalignment='right', fontsize=10, color=color)

# Aggiunta annotazione cedola
plt.annotate('Cedola -500',
             xy=(pre_cedola_index, values[pre_cedola_index]), xycoords='data',
             xytext=(pre_cedola_index - 10, values[pre_cedola_index] - 200), textcoords='data',
