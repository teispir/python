...
# Trova il primo indice in cui compare il valore 1504.48
investment_change_value = 1504.48
change_index = np.where(np.array(values) == investment_change_value)[0][0]

investment_change_value2 = 1216.72
change_index2 = np.where(np.array(values) == investment_change_value2)[0][0]

investment_change_value3 = 1228.36
change_index3 = np.where(np.array(values) == investment_change_value3)[0][0]

investment_change_value4 = 1345.48
change_index4 = np.where(np.array(values) == investment_change_value4)[0][0]

investment_change_value5 = 1397.48
change_index5 = np.where(np.array(values) == investment_change_value5)[0][0]
...
plt.axvline(x=change_index, color='orange', linestyle='--', linewidth=1.5, label='Stake 150')

# Aggiungi una linea verticale al punto di cambio investimento
plt.axvline(x=change_index2, color='green', linestyle='--', linewidth=1.5, label='Stake 20')

# Aggiungi una linea verticale al punto di cambio investimento
plt.axvline(x=change_index3, color='blue', linestyle='--', linewidth=1.5, label='Stake 30')

# Aggiungi una linea verticale al punto di cambio investimento
plt.axvline(x=change_index4, color='grey', linestyle='--', linewidth=1.5, label='Stake 40')

# Aggiungi una linea verticale al punto di cambio investimento
plt.axvline(x=change_index5, color='purple', linestyle='--', linewidth=1.5, label='Stake 50')

# Aggiungi una linea orizzontale rossa continua per il valore 1204
plt.axhline(y=1204, color='red', linestyle='-', linewidth=1.5, label='Soglia 1204')
...
# Imposta la griglia con valori interi
